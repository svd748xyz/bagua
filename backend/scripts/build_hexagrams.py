...
权威数据来源：通行本《周易》卦辞、爻辞、彖传、大象传。
- lines: 从下到上 6 爻，True=阳(▄▄▄▄▄) False=阴(▄▄  ▄▄)，索引 0=初爻
- 下卦/上卦各 3 爻，合成 6 爻后通过二进制映射到卦序(1-64)

卦的二进制编码规则（与 hexagram.py 的 line_to_hexagram_num 保持一致）：
    从下到上 6 爻拼成二进制，下卦为高3位、上卦为低3位 → 0..63，+1 = 卦序
传统伏羲八卦序与该映射的对应关系已内建在 _HEXAGRAM_TABLE 中。
"""
...
# 八卦基础信息：name/unicode/nature(属性)/binary
# binary 约定：bit0 = 最下爻（初爻），bit2 = 最上爻（三爻）。
# 这与 Unicode 卦符码点的位序一致（☳ 震 = U+2633 = 1阳在初）。
# 例：震 ☳ 初爻阳 → bit0=1 → 0b001；艮 ☶ 三爻阳 → 0b100；离 ☲ 中阳 → 0b010。
TRIGRAMS = {
    0b111: ("乾", "☰", "天", "健"),   # 三阳
    0b110: ("巽", "☴", "风", "入"),  # 初阴，二三阳
    0b101: ("离", "☲", "火", "丽"),  # 初三阳，中阴
...
# 八卦的权威 lines（自下而上 3 爻，1=阳 0=阴）。
# 这是 2000 年不变的常识，直接硬编码，不做任何二进制推导。
#   乾 ☰ 三阳          = 111
#   兑 ☰ 上缺(上阴)    = 110  （初二阳、上阴）
#   离 ☰ 中虚(中阴)    = 101  （初三阳、中阴）
#   震 ☰ 仰盂(初阳)    = 100  （初阳、二三阴）
...
#   艮 ☰ 覆碗(上阳)    = 001  （初二阴、上阳）
#   坤 ☷ 三阴          = 000
TRIGRAM_LINES: dict[str, str] = {
    "乾": "111", "兑": "110", "离": "101", "震": "100",
    "巽": "011", "坎": "010", "艮": "001", "坤": "000",
}
TRIGRAM_NATURE: dict[str, str] = {  # 八卦属性（用于大象传等校验参考）
    "乾": "天", "兑": "泽", "离": "火", "震": "雷",
    "巽": "风", "坎": "水", "艮": "山", "坤": "地",
}
TRIGRAM_UNICODE: dict[str, str] = {
    "乾": "☰", "兑": "☱", "离": "☲", "震": "☳",
    "巽": "☴", "坎": "☵", "艮": "☶", "坤": "☷",
}

# 64 卦权威数据表。
# 每条：(下卦名, 上卦名, 卦名, unicode, 卦辞, 彖传, 大象传)
# 上下卦名用汉字语义直接给出，由代码查 TRIGRAM_LINES 组合成 6 爻——
# 彻底脱离二进制数值的位序歧义。
HEXAGRAMS_TEXT = [
    # num=1 乾
    ("乾", "乾", "乾为天", "䷀",
    ...
    ("巽", "艮", "山风蛊", "䷑",
     "蛊：元亨，利涉大川。先甲三日，后甲三日。",
     "蛊，刚上而柔下，巽而止，蛊。蛊元亨，而天下治也。利涉大川，往有事也。先甲三日，后甲三日，终则有始，天行也。",
     "山下有风，蛊；君子以振民育德。"),
    # num=19 临（下兑上坤）
    ("兑", "坤", "地泽临", "䷒",
     "临：元，亨，利，贞。至于八月有凶。",
    ...
    ("离", "艮", "山火贲", "䷕",
     "贲：亨。小利有攸往。",
     "贲亨，柔来而文刚，故亨。分刚上而文柔，故小利有攸往。天文也；文明以止，人文也。观乎天文，以察时变；观乎人文，以化成天下。",
     "山下有火，贲；君子以明庶政，无敢折狱。"),
    # num=23 剥（下坤上艮）
    ("坤", "艮", "山地剥", "䷖",
     "剥：不利有攸往。",
    ...
    ("离", "兑", "泽火革", "䷰",
     "革：己日乃孚，元亨利贞，悔亡。",
     "革，水火相息，二女同居，其志不相得，曰革。己日乃孚，革而信之。文明以说，大亨以正，革而当，其悔乃亡。天地革而四时成，汤武革命，顺乎天而应乎人，革之时义大矣哉！",
     "泽中有火，革；君子以治历明时。"),
    # num=50 鼎（下巽上离）
    ("巽", "离", "火风鼎", "䷱",
     "鼎：元吉，亨。",
...
def build_hexagram_lines(lower_name: str, upper_name: str) -> list[bool]:
    """合成 6 爻（自下而上）。下卦为初二三爻，上卦为四五上爻。

    由语义化的上下卦名查 TRIGRAM_LINES 表组合，彻底脱离二进制位序歧义。
    """
    lower_bits = TRIGRAM_LINES[lower_name]   # 如 "100"
    upper_bits = TRIGRAM_LINES[upper_name]
    lines: list[bool] = []
    for ch in lower_bits:        # 下卦 3 爻 → 初、二、三爻
        lines.append(ch == "1")
    for ch in upper_bits:        # 上卦 3 爻 → 四、五、上爻
        lines.append(ch == "1")
    return lines



# 每卦 6 爻的爻辞。索引顺序：初、二、三、四、五、上。
# 对于乾、坤另含"用九""用六"（放在 extra 字段）。
# 这里给出通行本爻辞原文。
YAO_TEXTS: dict[int, list[str]] = {
    1: [  # 乾
        "初九：潜龙勿用。",
        "九二：见龙在田，利见大人。",
        "九三：君子终日乾乾，夕惕若厉，无咎。",
...
def build_all() -> list[dict]:
    assert len(HEXAGRAMS_TEXT) == 64, f"应有 64 卦，实际 {len(HEXAGRAMS_TEXT)}"
    out: list[dict] = []
    seen_lines: set[tuple[bool, ...]] = set()
    for idx, (lower_name, upper_name, name, unicode, judgement, tuan, image) in enumerate(
        HEXAGRAMS_TEXT, start=1
    ):
        assert idx == len(out) + 1
        lines = build_hexagram_lines(lower_name, upper_name)
        # 唯一性校验：64 卦的 lines 必须两两不同
        key = tuple(lines)
        assert key not in seen_lines, f"卦 {idx}({name}) 的 lines 与前卦重复"
        seen_lines.add(key)
        yao = YAO_TEXTS[idx]
        assert len(yao) == 6, f"卦 {idx} 爻辞数量不为 6"
        entry = {
            "num": idx,
            "name": name,
            "unicode": unicode,
            "lines": lines,  # 自下而上，True=阳
            "lower_trigram": lower_name,
            "upper_trigram": upper_name,
            "judgement": judgement,  # 卦辞
            "tuan": tuan,            # 彖传
            "image": image,          # 大象传
            "yao": yao,              # 6 条爻辞（初到上）
        }
        if idx in EXTRA_TEXT:
            entry["extra"] = EXTRA_TEXT[idx]
        out.append(entry)
    return out
...
def main() -> None:
    data = build_all()
    out_path = Path(__file__).resolve().parent.parent / "app" / "core" / "data" / "64hexagrams.json"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("w", encoding="utf-8") as f:
        json.dump({"hexagrams": data}, f, ensure_ascii=False, indent=2)
    print(f"已生成 {len(data)} 卦 -> {out_path}")
    # 自检
    nums = [h["num"] for h in data]
    assert nums == list(range(1, 65)), "卦序不连续"
    for h in data:
        assert h["name"], f"卦 {h['num']} 缺卦名"
        assert h["judgement"], f"卦 {h['num']} 缺卦辞"
        assert len(h["yao"]) == 6
    print("自检通过：64 卦完整，卦辞/6 爻辞齐全。")
